...
namespace TFCLPortal.Authorization
{
    public class AbpLoginResultTypeHelper : AbpServiceBase, ITransientDependency
    {
        ...
        public Exception CreateExceptionForFailedLoginAttemptCustomized(int result, string usernameOrEmailAddress, string tenancyName)
        {
            switch (result)
            {
                case 11:
                    return new UserFriendlyException(L("AndroidIdNotMatched"), "");
                default: // Can not fall to default actually. But other result types can be added in the future and we may forget to handle it
                    Logger.Warn("Unhandled login fail reason");
                    return new UserFriendlyException(L("LoginFailed"));
            }
        }
